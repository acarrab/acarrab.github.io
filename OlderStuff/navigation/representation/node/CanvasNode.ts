...
export default class CanvasNode {
    ...
    draw(cvs: HTMLCanvasElement, ctx: CanvasRenderingContext2D): boolean {
        ...
        ctx.font = '36px serif';
        ctx.font = Math.floor(radius / 3) + "px Roboto";
        ctx.fillStyle = this.internalNode.isPartOfPath() ?
            Artist.getFadingGradient(cvs, ctx, "orange") : 
            Artist.getFadingGradient(cvs, ctx, "blue");
        ctx.textAlign = "center";
        ctx.textBaseline = "middle";
        ctx.fillText(this.internalNode.getPage().name, position.x, position.y)
